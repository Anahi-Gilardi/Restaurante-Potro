...
-- Permisos y políticas para arca_config
GRANT SELECT, INSERT, UPDATE ON TABLE public.arca_config TO anon, authenticated;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'arca_config' AND policyname = 'allow_all_arca_config'
  ) THEN
    CREATE POLICY allow_all_arca_config ON public.arca_config FOR ALL TO public USING (true) WITH CHECK (true);
  END IF;
END $$;

-- Asegurar registro primario de configuración oficial ARCA
INSERT INTO public.arca_config (
  ...
  legal_name, trade_name, commercial_address, gross_income_number, activity_start_date
) VALUES (
  'primary', '27426946136', 2, 'produccion', 'monotributo',
