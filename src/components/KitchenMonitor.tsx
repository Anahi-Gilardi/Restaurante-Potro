...
export default function KitchenMonitor({
  ...
  const renderTicket = (p: Pedido, estado: Pedido['estado_comanda']) => {
    ...
    const headerTheme = {
      pendiente: 'bg-[#DB9C60]/90 text-[#FAF7F0]',
      en_cocina: 'bg-[#a0522d]/90 text-[#FAF7F0]',
      listo: 'bg-[#2e8b57]/90 text-[#FAF7F0]'
    }[estado];

    const btnTheme = {
      pendiente: 'bg-[#DB9C60] hover:bg-[#5d3a2e] text-[#FAF7F0] border-[#C8956A]/30 transition-all hover:scale-[1.02] active:scale-95 duration-200',
    ...
    return (
      ...
      >
        ...
        <div className="p-4 space-y-3 flex-1 flex flex-col justify-between overflow-hidden">
          <div className="space-y-2 overflow-y-auto pr-1 flex-1 max-h-[185px]">
            {p.items.map((it, idx) => (
              ...
              >
                ...
                <span className="text-[9px] uppercase font-black tracking-wider text-[#4b3621]/80 dark:text-stone-300 bg-[#e2dabf]/60 dark:bg-[#DB9C60]/40 px-2 py-0.5 rounded-md shrink-0">
          ...
          {p.observaciones && (
            <div className="bg-[#FAF7F0] dark:bg-[#DB9C60]/40 text-[#4b3621] dark:text-stone-200 text-xs p-3 rounded-xl border border-stone-200/50 dark:border-stone-800/40 italic font-medium leading-relaxed">
    ...
    return (
      <div className="space-y-4">
        <div className={`flex justify-between items-center p-4 rounded-t-xl border-b-[3px] ${headerClass}`}>
          ...
          <span className="bg-[#DB9C60] text-[#FAF7F0] text-[11px] font-black font-mono w-6 h-6 rounded-full flex items-center justify-center shadow-sm">
            {orders.length}
          </span>
        </div>
    ...
    <div className="space-y-5" id="kitchen-monitor-container">
      ...
      <div className="glass-panel rounded-[20px] p-5 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-4 border-b border-stone-200/30 dark:border-stone-800/40">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-[#DB9C60] text-[#FAF7F0] flex items-center justify-center text-xl shadow-sm border border-stone-200/30">
              📋
            </div>
          ...
          <span className="bg-[#DB9C60] text-[#FAF7F0] text-xs font-black py-1 px-3 rounded-full shadow-sm w-fit border border-[#FAF7F0]/10">
            {batchProduction.reduce((sum, item) => sum + item.cantidad, 0)} UNIDADES
          </span>
        </div>

        {batchProduction.length === 0 ? (
          <p className="text-xs text-[#4b3621]/60 dark:text-stone-400 italic text-center py-3 bg-[#e2dabf]/30 dark:bg-[#DB9C60]/20 rounded-xl">
          ...
          <div className="flex flex-wrap gap-3">
            {batchProduction.map((item, idx) => (
              ...
              >
                <span className="bg-[#DB9C60] text-[#FAF7F0] text-[11px] font-black w-7 h-7 rounded-full flex items-center justify-center">
                  {item.cantidad}
                </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {renderColumn(
          ...
          'bg-[#DB9C60]/10 dark:bg-[#DB9C60]/30 border-[#a0522d]/60',
          ordersPendientes
        )}
        {renderColumn(
          'en_cocina',
