...
export default function App() {
  ...
  const handleNavigate = (view: AppView) => {
    if (!canAccessView(activeUser.rol, view)) {
      toast.warning(`Acceso denegado. El rol "${activeUser.rol}" no tiene permiso para este módulo.`);
      setActiveView('home');
      return;
    }
    if (activeUser.rol.toLowerCase() === 'administrador' && (view === 'backups' || view === 'sistema')) {
      toast.error('Módulo restringido para el rol Administrador.');
      setActiveView('home');
      return;
    }
    setActiveView(view);
  ...
  const handleLoginSuccess = (operatorName?: string) => {
    ...
    const operator = requestedOperator
      || usuarios.find(usuario => usuario.activo !== false && usuario.rol === 'super_admi')
      || usuarios.find(usuario => usuario.activo !== false && usuario.rol === 'administrador')
      || usuarios.find(usuario => usuario.activo !== false);
          ...
          {activeView === 'home' && (
            ...
            <div key={activeView} className="animate-fadeIn">
              <HomeMenuModule
                ...
                canChangeUser={activeUser.rol === 'administrador' || activeUser.rol === 'super_admi'}
