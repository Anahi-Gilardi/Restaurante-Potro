...
const [minutosGlobal, setMinutosGlobal] = useState<number>(0);
  ...
  return (
    ...
    <div className="h-screen overflow-hidden bg-[#F5F1E9] flex font-sans text-stone-800 antialiased selection:bg-[#624A3E] selection:text-white">
      ...
      <aside
        className={`hidden lg:flex flex-col h-screen fixed left-0 top-0 bg-[#C8956A] text-[#3B1F10]/90 border-r border-[#A67550]/40 shrink-0 shadow-xl shadow-black/15 transition-all duration-300 z-30 ${
      ...
      >
        <div className="flex-1 overflow-y-auto scroll-passive overscroll-contain px-2 pt-2 space-y-1.5 sidebar-scroll-hidden">
          <div className={`flex items-center gap-2.5 py-1.5 px-1 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            {[
              ...
              return (
                <button key={item.id} id={`tab-${item.id}`}
                  ...
                  className={`w-full flex items-center gap-3 transition-all duration-200 cursor-pointer mx-1 rounded-xl ${
                    isActive
                ...
                >
                  ...
                  <span className={`text-[12px] font-bold tracking-wide leading-none whitespace-nowrap overflow-hidden transition-all duration-300 ${
                    isActive ? 'text-white' : 'text-[#3B1F10]/70'
                  } ${isSidebarCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
                    {item.label}
                  </span>
                  {isActive && (
                    <span className={`w-1.5 h-1.5 rounded-full bg-amber-300 shadow-sm shadow-amber-400/50 shrink-0 ${isSidebarCollapsed ? 'absolute top-2 right-2' : 'ml-auto'}`} />
                  )}
                </button>
              );
            })}
        ...
        <div className="shrink-0 border-t border-[#A67550]/40 bg-[#C8956A]">
          <div className="px-2 py-2">
            ...
            <button onClick={handleLogout}
              ...
              className={`w-full flex items-center gap-3 rounded-xl transition-all duration-200 cursor-pointer whitespace-nowrap text-[#7B2D12] hover:bg-[#7B2D12]/15 hover:text-[#5C1E0A] border border-transparent hover:border-[#7B2D12]/20 ${
                isSidebarCollapsed ? 'justify-center px-2 py-2.5' : 'px-3 py-2'
              }`}
            >
          ...
          <div className={`px-3 py-1.5 border-t border-[#A67550]/30 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
            <span className={`text-[8px] text-[#3B1F10]/30 font-mono tracking-wider whitespace-nowrap overflow-hidden transition-all duration-300 ${
              isSidebarCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'
            }`}>
              El Patrón Pro · v1.2.0
            </span>
            <button
              onClick={() => setIsSidebarCollapsed(c => !c)}
              title={isSidebarCollapsed ? 'Expandir' : 'Colapsar'}
              className="p-1 rounded-lg bg-[#3B1F10]/10 text-[#3B1F10]/70 hover:bg-[#3B1F10]/20 hover:text-[#3B1F10] cursor-pointer transition-colors"
            >
      ...
      {/* MAIN CONTENT AREA */}
      <main className={`flex-1 overflow-x-hidden p-2 sm:p-3 md:p-4 lg:p-6 pb-24 pt-16 lg:pt-2 max-w-[1600px] mx-auto w-full transition-all duration-300 ${
        isSidebarCollapsed ? 'lg:pl-[4.5rem]' : 'lg:pl-[17rem]'
      }`}>
