...
def page_menu() -> None:
    ...
    if _usando_supabase():
        st.success("🟢 Conectado a Supabase — los cambios se reflejan en la nube", icon="☁️")
    else:
        st.info("🟡 Modo local (SQLite) — configurá SUPABASE_URL y SUPABASE_ANON_KEY para sincronizar", icon="💾")

    _seccion_promocion()

    tab_menu_diario, tab_nuevo, tab_existentes = st.tabs(["Menú del Día 🌟", "Nuevo producto", "Productos existentes"])
